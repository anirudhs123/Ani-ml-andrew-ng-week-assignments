...
for j=1:m
  ...
  %J=J+(y(i)-h(i))*(y(i)-h(i));
   J=J-(ynet(i,j)*log (h2(i,j)) +(1-ynet(i,j))*log (1-h2(i,j)));
  endfor
endfor
J=J/m;
...
delta2=zeros(size(Theta1,1)+1,m);

  delta3=h2-ynet;
  delta2=(Theta2)'*delta3;
  delta2=delta2(2:size(delta2,1),:);
  delta2=delta2.*(h1).*(1-h1);
%  delta2(1,:)=0;
Theta1_grad = zeros(size(Theta1,1),size(Theta1,2));
Theta2_grad = zeros(size(Theta2,1),size(Theta2,2));


Theta2_grad=Theta2_grad+(delta3)*([ones(1,size(h1,2)); h1])';
%Theta2_grad=[ones(num_labels,1) Theta2_grad];
